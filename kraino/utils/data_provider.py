...
import numpy as np
import sys
sys.path.append('/home/kawhi/Desktop/vqa data/visual_turing_test-tutorial-master/kraino/utils')
...
def global_visual_features(
    ...
    visual_features = np.load(os.path.join(
        path, perception, 'blobs.' + layer + '.npy'), encoding="latin1", allow_pickle=True)
    ...
    if names_list is None or names_list==[]:
        ...
        tmp_feats = visual_features[list(visual_features.keys())[0]]
        ...
        'daquar-triples': {
            ...
            'perception':lambda train_or_test, names_list, 
                ...
                get_global_perception(
                    ...
                    perception=perception,
                    layer=layer, 
                    path = "/home/kawhi/Desktop/V/ex/data/daquar/visual_features"), 
